...
Choices on experimental design and analysis were made within the Bayesian framework of statistical inference (see \@ref(s:sample-size} and \Cref{s:bayes) for details).
...
This structure specification generates 24 test words, shown in \@ref(t:words).
...
kable(rr_1_tidy, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the Bayesian regression fitted to release-to-release duration (model 1, see \\@ref(s:rr)).") %>%
...
kable(rr_2_tidy, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the Bayesian regression fitted to release-to-release duration (model 2, see \\@ref(s:rr)).") %>%
kable_styling(font_size = 10)
```
...
kable(rr_3_tidy, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the Bayesian regression fitted to release-to-release duration and predictors from model 1 and 2 (model 3, see \\@ref(s:rr)).") %>%
kable_styling(font_size = 10)
```
...
kable(vow_4_tidy, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the Bayesian regression fitted to vowel duration (model 4, see \\@ref(s:vow)).") %>%
kable_styling(font_size = 10)
```

\@ref(tab:vow-4-table} reports the summary of model 4, while \Cref{fig:vow-4-intervals) shows the posterior distributions and credible intervals.
...
kable(clos_5_tidy, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the Bayesian regression fitted to closure duration (model 5, see \\@ref(s:clos)).") %>%
kable_styling(font_size = 10)
```
...
This section discusses in turn the results in relation to the release-to-release interval duration (\@ref(s:rr-disc}) and to vowel and closure durations (\Cref{s:vc-disc)) by comparing them with the hypotheses of this study.
\@ref(s:rr-gen-disc} synthesises and links these findings back to the articulatory grounding of the temporal properties of the release-to-release interval in mono- and disyllabic words (\Cref{s:gest)).
