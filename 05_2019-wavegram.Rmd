...
```{r load-data}
load("./cache/wavegam.Rdata")
```

\chapter{Modelling electroglottographic data with wavegrams and generalised additive mixed models [Paper IV]}
\label{p:wavegram}

\noindent\fbox{%
    \parbox{\textwidth}{%
        Coretta, Stefano. 2019. Modelling electroglottographic data with wavegrams and generalised additive mixed models. Manuscript. DOI: \url{https://doi.org/10.31219/osf.io/m623d}.
    }%
}
...
The wavegram technique has been proposed as a visualisation form of electroglottographic data which maintains the multi-dimensionality of the signal \citep{herbst2010}.
In this paper, generalised additive modelling \citep{hastie1986} is proposed as a means to statistically investigate glottal activity as derived from wavegrams.
...
A high frequency low voltage electrical current is sent through two electrodes which are in contact with the surface of the neck, one on each side of the thyroid cartilage (\Cref{fig:egg-setup}).
...
An example EGG signal is provided in \Cref{fig:egg}.
...
The ratio of the closed phase relative to the total cycle duration, the closed quotient, has been used as an index of phonation type \citep{scherer1987}.
Modal voice has higher closed quotient values than breathy voice, and lower values than creaky voice.
One method for the detection of the closing and opening instants is based on the first derivative of the EGG signal (the dEGG, see \Cref{fig:egg}).
...
The procedure for constructing a wavegram is given in \Cref{fig:wavegram}.
...
First, \Cref{s:pilot} presents results from a pilot study which informally evaluates the reliability of the proposed method.
\Cref{s:voicing} illustrates how to conduct a dynamical wavegram GAM analysis of dEGG data through a practical example in which the wavegrams of vowels followed by voiceless and voiced stops in Italian and Polish are compared.
...
The data \citep{coretta2017e, coretta2018m} and analysis scripts of this paper (\url{https://osf.io/3w8gh/}) can be found on the Open Science Framework.
...
The reader is referred to \citet{coretta2017e} for the documentation of the algorithms and the research data.
...
\Cref{fig:surface-p} shows the modelled wavegrams of modal and breathy tokens.
...
```{r surface-p, include=TRUE, fig.cap = "Fitted wavegram of modal and breathy phonation (Section 2). The horizontal lines represent the dEGG maximum (solid line) and minimum (dashed line).", out.extra="width=\\linewidth"}
lines <- tibble(
...
The data of this study is available on the Open Science Framework \citep{coretta2018m}.

```{r surface, include=TRUE, fig.cap = "Fitted wavegram of vowels followed by voiceless and voiced stops in Italian and Polish (Section 3).", out.extra="width=\\linewidth"}
contexts <- c(
...
\Cref{fig:surface} shows the modelled wavegrams of vowels followed by voiceless (left) and voiced stops (right), in Italian (top) and Polish (bottom).
...
Focussing now on the second half of the vowel, the wavegrams in \Cref{fig:surface} show a pattern that is symmetrical to that observed in the first half.
Namely, glottal opening increases towards the end of the vowel (also see \citealt{halle1967a}).
...
The approximate time of the change onset is represented by the vertical dashed lines in \Cref{fig:surface}.
...
\citet{nichasaide1985} argues that pre-aspiration develops as a means to enhance discriminability in geminate stops by increasing their overall duration.
