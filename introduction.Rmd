...
## Exploratory study of the voicing effect in Italian and Polish
...
Electroglottography \citep{fabre1957} is a technique that measures the size of contact between the vocal folds (the Vocal Folds Contact Area, VFCA).
...
The impedance of the current is directly correlated with VFCA, while its amplitude is inversely correlated \citep{titze1990}.
...
Two separate laptops were used for the acquisition of the ultrasound and EGG recordings.
The ultrasound unit was plugged into one laptop.
A P-Stretch unit (used for signal synchronisation) and the ultrasound probe were directly connected to the ultrasound unit.
The P-Stretch unit and a microphone fed a pre-amplifier system, which was plugged into the ultrasound laptop.
A second microphone and the electrodes were connected to the EGG unit, which was plugged into the second laptop.
A TELEMED Echo Blaster 128 system was used for ultrasonography and a Glottal Enterprises EG2-PCX2 unit for EGG.
The subject wore a headset (not shown in \Cref{f:setup}) which holds the ultrasound probe in position (allowing free head movement), and the velcro strap with the EGG electrodes, located on each side of the thyroid cartilage, at the level of the glottis.
The microphones were clipped to the headset on either side, at identical height.

Ultrasound and EGG inputs are acquired and recorded in separate laptops by means of, respectively, Articulate Assistant Advanced (AAA, \citealt{articulate2011}) and Praat \citep{boersma2016}.
...
Synchronisation is achieved through the cross-correlation of the audio signals from both sources \citep{grimaldi2008}.
