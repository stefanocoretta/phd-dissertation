...
library(kableExtra)
source("./cache.R")
dir.create("./data/cache/04_2018-tra", showWarnings = FALSE)
```
...
```{r tra-lm-table}
kable(tidy(tra_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Voicing = voiced", "Speech rate (centr.)", "Vowel = /o/", "Vowel = /u/")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), linesep = "", digits = 4, caption = "Summary of the linear mixed-effects model fitted to tongue root position at vowel offset (see \\@ref(s:tra-lm))") %>%
kable_styling(font_size = 10)
```

### Tongue root position during V1 {#s:trp-v1}

```{r tra-gam, include=FALSE}
tra_gam <- cache_rds({
  bam(
  trp ~
    c2_phonation_o +
...
}, "./data/cache/04_2018-tra/tra_gam")

rho <- start_value_rho(tra_gam)

tra_gam_ar <-cache_rds({
  bam(
  trp ~
    c2_phonation_o +
    s(speech_rate_c) +
...
}, "./data/cache/04_2018-tra/tra_gam_ar")

tra_gam_ar_0 <- cache_rds({
  bam(
  trp ~
    # c2_phonation_o +
    s(speech_rate_c) +
...
}, "./data/cache/04_2018-tra/tra_gam_ar_0")
...
}, "./data/cache/04_2018-tra/smooths")

difference <- cache_rds({
  plot_difference(tra_gam_ar, proportion, list(c2_phonation_o = c("voiced", "voiceless"))) +
  scale_x_continuous(labels = NULL, name = element_blank())
}, "./data/cache/04_2018-tra/difference")
...
}, "./data/cache/04_2018-tra/tra_lm_2")

summary(tra_lm_2)
```

```{r Figure4, include=TRUE, fig.cap="Raw data, regression lines, and 95\\% confidence intervals of the correlation between vowel duration and tongue root position for each vowel (/a/, /o/, and /u/). The regression line and confidence intervals are from a mixed-effects model (see \\@ref(s:trp-vdur)).", out.extra="width=\\linewidth"}
...
```{r tra-lm-2-table}
kable(tidy(tra_lm_2, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "V1 duration (centr.)", "Speech rate (centr.)", "Vowel = /o/", "Vowel = /u/", "V1 duration × /o/", "V1 duration × /u/")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), linesep = "", digits = 4, caption = "Summary of the linear mixed-effects model for testing the correlation between tongue root position and V1 duration  (see \\@ref(s:trp-vdur))") %>%
kable_styling(font_size = 10)
```

### Tongue root position during V1 as a function of V1 duration {#s:trp-v1-dur}

```{r tra-gam-2, include=FALSE}
tra_gam_2 <- cache_rds({
  bam(
  trp ~
    s(v1_duration) +
...
}, "./data/cache/04_2018-tra/tra_gam_2")

rho <- start_value_rho(tra_gam_2)

tra_gam_ar_2 <- cache_rds({
  bam(
  trp ~
    s(v1_duration) +
    s(proportion) +
...
}, "./data/cache/04_2018-tra/tra_gam_ar_2")

tra_gam_ar_0_2 <- cache_rds({
  bam(
  trp ~
    s(v1_duration) +
    s(proportion) +
...
}, "./data/cache/04_2018-tra/tra_gam_ar_0_2")
```
...
kable(
  ...
  linesep = "",
  digits = 4,
  caption = "Summary of the GAM model fitted to tongue root position during V1 as a function of V1 duration (see \\@ref(s:trp-v1-dur))"
) %>%
kable_styling(font_size = 10)
```

```{r Figure5, include=TRUE, fig.cap="Predicted tongue root position during vowels at 4 exemplifying values of vowel duration, with 95\\% confidence intervals. Predictions from a GAMM (see \\@ref(s:trp-v1-dur)).", out.extra="width=\\linewidth"}
figure5 <- cache_rds({
...
}, "./data/cache/04_2018-tra/figure5")
print(figure5)
```
...
```{r Figure7, include=TRUE, fig.cap="By-speaker raw mean difference in tongue root position between voiceless and voiced stops at closure onset (in millimetres). The horizontal segments are the standard errors of the mean differences.", out.extra="width=\\linewidth"}
trp_difference %>%
...
}, "./data/cache/04_2018-tra/tra_gam_s")

rho <- start_value_rho(tra_gam_s)

tra_gam_s_ar <- cache_rds({
  bam(
  trp ~
    s(speech_rate_c) +
    s(proportion) +
...
}, "./data/cache/04_2018-tra/tra_gam_s_ar")
```

```{r Figure8, include=TRUE, fig.cap="Predicted tongue root position during vowels followed by voiceless and voiced stops for each speaker. Predicted from a GAMM (see text). Note the different scales on the y-axis.", out.extra="width=\\linewidth", fig.height=8, dependson="tra-gam-s-ar"}
figure8 <- cache_rds({
  ...
  scale_colour_discrete(name = "C2 voicing") +
  scale_linetype_discrete(name = "C2 voicing")
}, "./data/cache/04_2018-tra/figure8")
print(figure8)
```
