...
<!-- \begin{figure*}
    ...
    \caption{Schematic representation of the equipment setup (@articulate2011}). The system is described in details in \@ref(s:equipment}.)
    \label{fig:uti-setup}
\end{figure*} -->
...
The resulting words are nonce words, with a few exceptions, and they were presented in the languages' respective writing conventions (see \@ref(t:targets)).
...
```{r tra-lm-table}
kable(tidy(tra_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Voicing = voiced", "Speech rate (centr.)", "Vowel = /o/", "Vowel = /u/")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), format = "latex", booktabs = TRUE, linesep = "", digits = 4, caption = "Summary of the linear mixed-effects model fitted to tongue root position at vowel offset (see \\@ref(s:tra-lm))") %>%
...
```{r tra-lm-2-table}
kable(tidy(tra_lm_2, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "V1 duration (centr.)", "Speech rate (centr.)", "Vowel = /o/", "Vowel = /u/", "V1 duration × /o/", "V1 duration × /u/")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), format = "latex", booktabs = TRUE, linesep = "", digits = 4, caption = "Summary of the linear mixed-effects model for testing the correlation between tongue root position and V1 duration  (see \\@ref(s:trp-vdur))") %>%
kable_styling(font_size = 10)
```
...
```{r Figure5, include=TRUE, fig.cap="Predicted tongue root position during vowels at 4 exemplifying values of vowel duration, with 95\\% confidence intervals. Predictions from a GAMM (see \\@ref(s:trp-v1-dur}).", out.extra="width=\\linewidth", cache=TRUE)
...
The results presented in \@ref(s:results} and discussed in \Cref{s:discussion) are group-level patterns of the population sampled in the present study.
