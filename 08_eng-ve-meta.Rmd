...
```{r data}
stressed <- read_csv("./datasets/stressed.csv") %>%
  mutate(
...
\chapter{Bayesian meta-analysis of the voicing effect in English}
\label{a:ve-eng}

A Bayesian meta-analysis of the English voicing effect was run on the basis of 11 estimated posterior distributions extracted from 9 different publications, following the procedures discussed in \citet{nicenboim2018a}.
...
A data set with the mean estimates and estimated standard errors from these 11 posterior distributions (\Cref{tab:est}) has then been used to fit a further Bayesian measurement error model.
...
kable(est, format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Bayesian estimates of the voicing effect in indvidual studies.") %>%
...
\Cref{fig:intervals-plot} is a visual aid to the summury, and shows a variety of credible intervals of the estimates from the model.
...
A visual representation of the meta-analytical distributions is given in \Cref{fig:syl-plot}.
...
\Cref{fig:origin-shrunk-plot} shows the mean estimates (the points) of the voicing effect with 95% CIs (the horizontal segments) for each of the 11 studies.
...
\Cref{fig:funnel-plot} is a funnel plot, which can be used to visually check whether the sample suffers from publication bias.
