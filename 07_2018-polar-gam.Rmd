...
stimuli_it <- read_csv("./data/datasets/stimuli-it.csv")

stimuli_pl <- read_csv("./data/datasets/stimuli-pl.csv")
...
tongue_it01 <- read_aaa("./data/datasets/it01-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
...
tongue_it02 <- read_aaa("./data/datasets/it02-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_it) %>%
...
tongue_it03 <- read_aaa("./data/datasets/it03-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_it) %>%
...
tongue_it04 <- read_aaa("./data/datasets/it04-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_it) %>%
...
tongue_it07 <- read_aaa("./data/datasets/it07-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_it) %>%
...
tongue_it11 <- read_aaa("./data/datasets/it11-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_it) %>%
  filter(c1_phonation == "voiceless", label %in% c("max_TT", "max_TD")) %>%
...
tongue_pl02 <- read_aaa("./data/datasets/pl02-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
...
tongue_pl03 <- read_aaa("./data/datasets/pl03-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_pl) %>%
  filter(c1_phonation == "voiceless", label %in% c("max_TT", "max_TD")) %>%
...
tongue_pl04 <- read_aaa("./data/datasets/pl04-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_pl) %>%
  filter(c1_phonation == "voiceless", label %in% c("max_TT", "max_TD")) %>%
...
tongue_pl05 <- read_aaa("./data/datasets/pl05-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_pl) %>%
  filter(c1_phonation == "voiceless", label %in% c("max_TT", "max_TD")) %>%
...
tongue_pl06 <- read_aaa("./data/datasets/pl06-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_pl) %>%
...
tongue_pl07 <- read_aaa("./data/datasets/pl07-tongue-cart.tsv", columns)  %>%
  na.omit() %>%
  mutate(word = word(prompt, 2)) %>%
  left_join(y = stimuli_pl) %>%
...
```{r Figure02, include=TRUE, fig.cap = "Estimated tongue contours of IT01 depending on C2 place, vowel and C2 voicing.", out.extra="width=\\linewidth"}
tongue_it01 %>%
  ggplot(aes(X, Y)) +
...
```{r Figure03, include=TRUE, fig.cap = "Estimated tongue contours of IT01 depending on C2 place, vowel and C2 voicing.", out.extra="width=\\linewidth"}
tongue_it01 %>%
  ggplot(aes(X, Y)) +
...
}, "./data/cache/07_2018-polar-gam/it01_gam")
```

```{r it01-gam-2, echo=TRUE, eval=FALSE}
it01_gam <- polar_gam(
  Y ~
    vc_voicing +            # parametric term
    s(X) +                  # reference smooth
...
```{r it01-gam-plot-code, echo=TRUE}
plot_polar_smooths(
  it01_gam,
  X,
  voicing,
...
```{r Figure04, include=TRUE, fig.cap = "Estimated tongue contours of IT01 depending on C2 place, vowel and C2 voicing.", out.extra="width=\\linewidth"}
plot_polar_smooths(
  it01_gam,
  X,
  voicing,
...
```{r it01-gam-0, echo=FALSE}
it01_gam_0 <- cache_rds({
  polar_gam(
  Y ~
...
}, "./data/cache/07_2018-polar-gam/iti01_gam_0")
```

```{r it01-gam-0-2, echo=TRUE, eval=FALSE}
...
```{r Figure05, include=TRUE, fig.cap = "Difference smooth of voiceless vs voiced stops in the context of /a/ (top) and /u/ (bottom)."}
plot_difference(
  it01_gam,
  X,
...
```{r it01-gam-ar, message=FALSE}
it01_rho <- start_value_rho(it01_gam)

it01_gam_ar <- cache_rds({
  polar_gam(
  Y ~
    vc_voicing +
    s(X) +
...
}, "./data/cache/07_2018-polar-gam/it01_gam_ar")
```

```{r Figure06, include=TRUE, fig.cap = "Autocorrelation plots of a model fitted without (top) and with (bottom) a first-order autoregressive model (AR1)."}
...
```{r it01-gam-ar-2, message=FALSE, include=FALSE}
it01_gam_ar_0 <- cache_rds({
  polar_gam(
  Y ~
    s(X),
  data = tongue_it01,
...
}, "./data/cache/07_2018-polar-gam/it01_gam_ar_0")

compareML(it01_gam_ar_0, it01_gam_ar)
```

```{r it02-gam-ar, message=FALSE, include=FALSE}
it02_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_it02,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/it02_gam")

it02_rho <- start_value_rho(it02_gam)

it02_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/it02_gam_ar")

it02_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_it02,
  method = "ML",
  rho = it02_rho,
  AR_start = tongue_it02$start_event
)}, "./data/cache/07_2018-polar-gam/it02_gam_ar_0")

compareML(it02_gam_ar_0, it02_gam_ar)
```

```{r it03-gam-ar, message=FALSE, include=FALSE}
it03_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_it03,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/it03_gam")

it03_rho <- start_value_rho(it03_gam)

it03_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/it03_gam_ar")

it03_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_it03,
  method = "ML",
  rho = it03_rho,
  AR_start = tongue_it03$start_event
)}, "./data/cache/07_2018-polar-gam/it03_gam_ar_0")

compareML(it03_gam_ar_0, it03_gam_ar)
```

```{r it04-gam-ar, message=FALSE, include=FALSE}
it04_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_it04,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/it04_gam")

it04_rho <- start_value_rho(it04_gam)

it04_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/it04_gam_ar")

it04_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_it04,
  method = "ML",
  rho = it04_rho,
  AR_start = tongue_it04$start_event
)}, "./data/cache/07_2018-polar-gam/it04_gam_ar_0")

compareML(it04_gam_ar_0, it04_gam_ar)
```

```{r it07-gam-ar, message=FALSE, include=FALSE}
it07_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_it07,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/it07_gam")

it07_rho <- start_value_rho(it07_gam)

it07_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/it07_gam_ar")

it07_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_it07,
  method = "ML",
  rho = it07_rho,
  AR_start = tongue_it07$start_event
)}, "./data/cache/07_2018-polar-gam/it07_gam_ar_0")

compareML(it07_gam_ar_0, it07_gam_ar)
```

```{r it11-gam-ar, message=FALSE, include=FALSE}
it11_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_it11,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/it11_gam")

it11_rho <- start_value_rho(it11_gam)

it11_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/it11_gam_ar")

it11_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_it11,
  method = "ML",
  rho = it11_rho,
  AR_start = tongue_it11$start_event
)}, "./data/cache/07_2018-polar-gam/it11_gam_ar_0")

compareML(it11_gam_ar_0, it11_gam_ar)
```

```{r pl02-gam-ar, message=FALSE, include=FALSE}
pl02_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_pl02,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/pl02_gam")

pl02_rho <- start_value_rho(pl02_gam)

pl02_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/pl02_gam_ar")

pl02_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl02,
  method = "ML",
  rho = pl02_rho,
  AR_start = tongue_pl02$start_event
)}, "./data/cache/07_2018-polar-gam/pl02_gam_ar_0")

compareML(pl02_gam_ar_0, pl02_gam_ar)
```

```{r pl03-gam-ar, message=FALSE, include=FALSE}
pl03_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_pl03,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/pl03_gam")

pl03_rho <- start_value_rho(pl03_gam)

pl03_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/pl03_gam_ar")

pl03_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl03,
  method = "ML",
  rho = pl03_rho,
  AR_start = tongue_pl03$start_event
)}, "./data/cache/07_2018-polar-gam/pl03_gam_ar_0")

compareML(pl03_gam_ar_0, pl03_gam_ar)
```

```{r pl04-gam-ar, message=FALSE, include=FALSE}
pl04_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_pl04,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/pl04_gam_ar")

pl04_rho <- start_value_rho(pl04_gam)

pl04_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
pl04_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl04,
  method = "ML",
  rho = pl04_rho,
  AR_start = tongue_pl04$start_event
)}, "./data/cache/07_2018-polar-gam/pl04_gam_ar_0")

compareML(pl04_gam_ar_0, pl04_gam_ar)
```

```{r pl05-gam-ar, message=FALSE, include=FALSE}
pl05_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  data = tongue_pl05,
  method = "ML"
)}, "./data/cache/07_2018-polar-gam/pl05_gam")

pl05_rho <- start_value_rho(pl05_gam)

pl05_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
...
)}, "./data/cache/07_2018-polar-gam/pl05_gam_ar")

pl05_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl05,
  method = "ML",
  rho = pl05_rho,
  AR_start = tongue_pl05$start_event
)}, "./data/cache/07_2018-polar-gam/pl05_gam_ar_0")

compareML(pl05_gam_ar_0, pl05_gam_ar)
```

```{r pl06-gam-ar, message=FALSE, include=FALSE}
pl06_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  ...
  method = "ML",
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl06_gam")

pl06_rho <- start_value_rho(pl06_gam)

pl06_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  ...
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl06_gam_ar")

pl06_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl06,
  method = "ML",
  ...
  AR_start = tongue_pl06$start_event,
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl06_gam_ar_0")

compareML(pl06_gam_ar_0, pl06_gam_ar)
```

```{r pl07-gam-ar, message=FALSE, include=FALSE}
pl07_gam <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  ...
  method = "ML",
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl07_gam")

pl07_rho <- start_value_rho(pl07_gam)

pl07_gam_ar <- cache_rds({polar_gam(
  Y ~
    vc_voicing +
    s(X) +
    s(X, by = vc_voicing),
  ...
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl07_gam_ar")

pl07_gam_ar_0 <- cache_rds({polar_gam(
  Y ~
    s(X),
  data = tongue_pl07,
  method = "ML",
  ...
  AR_start = tongue_pl07$start_event,
  fan_lines = c(16, 19)
)}, "./data/cache/07_2018-polar-gam/pl07_gam_ar_0")
...
```{r Figure07, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT01.", out.width=".8\\textwidth"}
plot_polar_smooths(it01_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure08, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT02.", out.width=".8\\textwidth"}
plot_polar_smooths(it02_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure09, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT03.", out.width=".8\\textwidth"}
plot_polar_smooths(it03_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure10, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT04.", out.width=".8\\textwidth"}
plot_polar_smooths(it04_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure11, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT07.", out.width=".8\\textwidth"}
plot_polar_smooths(it07_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure12, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT11.", out.width=".8\\textwidth"}
plot_polar_smooths(it11_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure13, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL02.", out.width=".8\\textwidth"}
plot_polar_smooths(pl02_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure14, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL03.", out.width=".8\\textwidth"}
plot_polar_smooths(pl03_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure15, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL04.", out.width=".8\\textwidth"}
plot_polar_smooths(pl04_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure16, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL05.", out.width=".8\\textwidth"}
plot_polar_smooths(pl05_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure17, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL06.", out.width=".8\\textwidth"}
plot_polar_smooths(pl06_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r Figure18, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL07.", out.width=".8\\textwidth"}
