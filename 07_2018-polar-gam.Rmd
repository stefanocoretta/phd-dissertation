...
\chapter{Assessing mid-sagittal tongue contours in polar coordinates using generalised additive (mixed) models}
\label{a:polar-gams}

\noindent\fbox{%
    \parbox{\textwidth}{%
        Coretta, Stefano. 2019. Assessing mid-sagittal tongue contours in polar coordinates using generalised additive (mixed) models. Manuscript. DOI: \url{https://doi.org/10.31219/osf.io/q6vzb}.
    }%
}
...
This "echo" is captured by the transducer and translated into an image like the one shown in \Cref{fig:uti} (for a technical description, see \citealt{stone2005}).
...
\citet{mielke2015} and \citet{heyne2015, heyne2015a} have shown that using polar coordinates of tongue contours rather than Cartesian coordinates brings several benefits, among which reduced variance at the edges of the tongue contour.
...
This issue is illustrated in \Cref{fig:Figure02}.
...
\Cref{fig:Figure03} shows a more appropriate (artefact-free) way of representing individual tongue contours.
...
Synchronised audio and ultrasound tongue imaging data have been recorded from 11 speakers of Italian and 6 speakers of Polish while reading a series of controlled sentences (only 6 of the 11 Italian speakers are analysed here, see \Cref{s:comptrp}).
...
The output of `plot_polar_smooths()` is shown in \Cref{fig:Figure04}.
...
\Cref{fig:Figure05} illustrates the use of difference smooths with the difference smooths of voiceless vs voiced coronal stops when the vocalic context is /a/ or /u/.
...
Based on the predicted smooths sown in \Cref{fig:Figure04}, we can argue that, in the context of coronal consonants, the root is more advanced in voiced relative to voiceless stops (when the vowel is either /a/ or /u/), and that the dorsum is also somewhat retracted in voiced stops if the vowel is /u/.

```{r Figure05, include=TRUE, fig.cap = "Difference smooth of voiceless vs voiced stops in the context of /a/ (top) and /u/ (bottom).", fig.show="hold",out.width=".7\\linewidth", out.height="6cm"}
plot_difference(
  it01_gam,
  X,
...
\Cref{fig:Figure06} show the autocorrelations in the residuals without (top) and with (bottom) an AR1 model.
...
```{r Figure06, include=TRUE, fig.cap = "Autocorrelation plots of a model fitted without (top) and with (bottom) a first-order autoregressive model (AR1).", fig.show="hold",out.width=".7\\linewidth", out.height="6cm"}
...
\Cref{fig:Figure07} to \Cref{fig:Figure18} show an appreciable degree of variation across speakers and phonological contexts in relation to the differences in tongue shapes between voiceless and voiced stops (IT07 and Pl05 both miss data from /u/ due to the poor quality of the ultrasonic image for this vowel).
