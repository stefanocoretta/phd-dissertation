...
load("./data/datasets/duration_data.Rdata")
...
Most are nonce words, although inevitably some combinations produce real words both in Italian (4 words) and Polish (2 words, see \@ref(tab:targetsR)).
...
```{r segmentationa, fig.cap="Segmentation example of the word *pata* uttered by the Italian speaker IT09 (the times on the *x*-axis refer to the times in the concatenated audio file)."}
knitr::include_graphics(c("./img/Figure1a.png"))
```

```{r segmentationb, fig.cap="Segmentation example of the word *pada* uttered by the Italian speaker IT09 (the times on the *x*-axis refer to the times in the concatenated audio file)."}
knitr::include_graphics("./img/Figure1b.png")
```
...
}, "./data/cache/02_2018-relrel/vow_lm")

summary(vow_lm)
```

```{r FigureR-2, include=TRUE, fig.cap = "Raw data and boxplots of the duration in milliseconds of vowels in Italian (top row) and Polish (bottom row), for the vowels /a, o, u/ when followed by a voiceless or voiced stop.", out.extra="width=\\linewidth"}
figurer_2 <- cache_rds({
  durations_filtered %>%
...
}, "./data/cache/02_2018-relrel/figurer_2")
print(figurer_2)
```
...
vow_descr <- cache_rds({
  ...
  summarise(mean = mean(v1_duration, na.rm = TRUE), sd = sd(v1_duration, na.rm = TRUE), .groups = "drop")
}, "./data/cache/02_2018-relrel/vow_descr")
```
...
}, "./data/cache/02_2018-relrel/clo_lm")

summary(clo_lm)
```

```{r FigureR-3, echo=FALSE, include=TRUE, fig.cap = "Raw data and boxplots of closure duration in milliseconds of voiceless and voiced stops in Italian (top row) and Polish (bottom row) when preceded by the vowels /a, o, u/.", out.extra="width=\\linewidth"}
figurer_3 <- cache_rds({
    durations_filtered %>%
...
}, "./data/cache/02_2018-relrel/figurer_3")
print(figurer_3)
```

```{r clos-descr}
clos_descr <- cache_rds({
  durations_filtered %>%
  group_by(language, c2_phonation) %>%
  summarise(mean = mean(c2_clos_duration, na.rm = TRUE), sd = sd(c2_clos_duration, na.rm = TRUE), .groups = "drop")
}, "./data/cache/02_2018-relrel/clos_descr")
```
...
}, "./data/cache/02_2018-relrel/vow_clo_lm")
summary(vow_clo_lm)
```

```{r FigureR-4, include=TRUE, fig.cap = "Raw data, estimated regression lines, and 95 per cent confidence intervals of the effect of closure duration on vowel duration for the vowels /a, o, u/ (from a mixed-effects model fitted to data pooled from Italian and Polish, see text for details).", out.extra="width=\\linewidth"}
figurer_4 <- cache_rds({
...
}, "./data/cache/02_2018-relrel/figurer_4")
print(figurer_4)
```
...
```{r vow-clo-table}
kable(tidy(vow_clo_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Closure dur.", "Vowel = /o/", "Vowel = /u/", "Speech rate", "Closure × /o/", "Closure × /u/", "C2 closure × sp. rate", "/o/ × sp. rate", "/u/ × sp. rate", "Closure × /o/ × sp. rate", "Closure × /u/ × sp. rate")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")),  linesep = "", digits = 2, caption = "Summary of the linear mixed-effects model for testing the correlation between vowel and closure duration (see \\@ref(s:vcduration)).") %>%
kable_styling(font_size = 8, latex_options = "hold_position")
```
...
}, "./data/cache/02_2018-relrel/word_lm")

word_lm_null <- cache_rds({
  lme4::lmer(
  word_duration ~
    # c2_phonation +
    vowel +
...
}, "./data/cache/02_2018-relrel/word_lm_null")

word_bf <- round(exp((BIC(word_lm) - BIC(word_lm_null)) / 2))
```

```{r word-descr}
word_descr <- cache_rds({
  word_filtered %>%
  group_by(language, c2_phonation) %>%
  summarise(mean = mean(word_duration, na.rm = TRUE), sd = sd(word_duration, na.rm = TRUE), .groups = "drop")
}, "./data/cache/02_2018-relrel/word_descr")
```
...
```{r FigureR-5, include=TRUE, fig.cap="Raw data and boxplots of the duration in milliseconds of the release-to-release interval in Italian (left) and Polish (right) when C2 is voiceless or voiced.", out.extra="width=\\linewidth", warning=FALSE}
figurer_5 <- cache_rds({
  rel_filtered %>%
...
}, "./data/cache/02_2018-relrel/figurer_5")
print(figurer_5)
```
...
}, "./data/cache/02_2018-relrel/rr_lm")

rr_lm_null <- cache_rds({
  lme4::lmer(
  rel_rel ~
    # c2_phonation +
    vowel +
...
}, "./data/cache/02_2018-relrel/rr_lm_null")
...
}, "./data/cache/02_2018-relrel/rr_descr")
```
...
```{r FigureR-6, include=TRUE, fig.cap = "By-speaker random coefficients and error bars for the effect of C2 voicing on vowel duration, extracted from a mixed-effect model (Section 3.1).", out.extra="width=\\linewidth", warning=FALSE}
figurer_6 <- cache_rds({
...
}, "./data/cache/02_2018-relrel/figurer_6")
print(figurer_6)
```
...
\@ref(fig:FigureR-6) shows the random coefficients of the effect of C2 voicing on vowel duration for the individual speakers, extracted from the mixed-effects model presented in \@ref(s:vduration).
...
```{r lmer-check, eval=FALSE, include=FALSE}
qqnorm(resid(vow_lm))
plot(vow_lm)
qqnorm(resid(clo_lm))
plot(clo_lm)
...
```{r variation, eval=FALSE, include=FALSE}
...
```{r vor-bf, eval=FALSE, include=FALSE}
vor_lm <- lme4::lmer(
  vor ~
    c2_phonation +
    vowel +
...
```{r vv-bf, eval=FALSE, include=FALSE}
vv_lm <- lme4::lmer(
  v_v ~
    c2_phonation +
    vowel +
