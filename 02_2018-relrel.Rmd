...
# An exploratory study of voicing-related differences in vowel duration as compensatory temporal adjustment in Italian and Polish [Paper I]
...
Most are nonce words, although inevitably some combinations produce real words both in Italian (4 words) and Polish (2 words, see \@ref(t:targetsR)).
The lexical stress of the target words was placed by speakers of both Italian and Polish on V\textsubscript{1}, as intended.

Table: (\#tab:targetsR) Target words. Asterisks indicate real words.
...
<!-- Possible influences of the semantic content of the frame sentence on duration estimates will be discussed in \@ref(s:discussionR). -->
...
Table: (\#tab:dur-measures) Criteria for the identification of acoustics landmarks.
...
| consonant release | (C1/C2 release) | Automatic detection + manual correction [@ananthapadmanabha2014] |
...
The outcome of the automatic annotation was manually corrected for the relevant boundaries, according to the criteria in \@ref(t:dur-measures) based on @machac2009.
...
\@ref(fig:segmentation) shows an example of the segmentation of /pata/ (a) and /pada/ (b) from an Italian speaker.
...
```{r segmentation, fig.cap="Segmentation example of the words *pata* and *pada* uttered by the Italian speaker IT09 (the times on the *x*-axis refer to the times in the concatenated audio file)."}
knitr::include_graphics(c("./img/Figure1a.png", "./img/Figure1b.png"))
```
...
\@ref(fig:FigureR-2) shows boxplots and raw data of vowel duration for the three vowels /a, o, u/ when followed by voiceless or voiced stops in Italian and Polish.
...
The following terms are significant according to *t*-tests with Satterthwaite's approximation to degrees of freedom (\@ref(tab:vow-table)): C2 voicing, C2 place, vowel, language, and speech rate.
...
```{r vow-table}
kable(tidy(vow_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Voicing = voiced", "Vowel = /o/", "Vowel = /u/", "Lang = Polish", "Place = velar", "Speech rate", "Voiced × /o/", "Voiced × /u/", "Voiced × Polish", "/o/ × Polish", "/u/ × Polish", "Voiced × /o/ × Polish", "Voiced × /u/ × Polish")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the linear mixed-effects model fitted to vowel duration (see \\@ref(s:vduration)).") %>%
kable_styling(font_size = 8, latex_options = "hold_position")
```

### Consonant closure duration {#s:cduration}
...
\@ref(fig:FigureR-3) illustrates stop closure durations with boxplots and individual raw data points.
...
C2 voicing, C2 place, and speech rate are significant (\@ref(tab:clo-table)).
...
```{r clo-table}
kable(tidy(clo_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Voicing = voiced", "Vowel = /o/", "Vowel = /u/", "Lang = Polish", "Place = velar", "Speech rate", "Voiced × /o/", "Voiced × /u/", "Voiced × Polish", "/o/ × Polish", "/u/ × Polish", "Voiced × /o/ × Polish", "Voiced × /u/ × Polish")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the linear mixed-effects model fitted to closure duration (see \\@ref(s:cduration)).") %>%
kable_styling(font_size = 8, latex_options = "hold_position")
```
...
\@ref(fig:FigureR-4) shows for each vowel /a, o, u/ the individual data points and the regression lines with 95\% confidence intervals extracted from the mixed-effects model.

```{r vow-clo-table}
kable(tidy(vow_clo_lm, effect = "fixed", conf.int = TRUE) %>% select(-effect) %>% mutate(term = c("Intercept", "Closure dur.", "Vowel = /o/", "Vowel = /u/", "Speech rate", "Closure × /o/", "Closure × /u/", "C2 closure × sp. rate", "/o/ × sp. rate", "/u/ × sp. rate", "Closure × /o/ × sp. rate", "Closure × /u/ × sp. rate")) %>% rename(Predictor = term, Estimate = estimate, SE = std.error, `t-value` = statistic, `p-value` = p.value, `CI low` = conf.low, `CI up` = conf.high) %>% select(Predictor, Estimate, SE, `CI low`, `CI up`, df, `t-value`, `p-value`) %>% mutate(`< α` = ifelse(`p-value` < 0.05, "*", "")), format = "latex", booktabs = TRUE, linesep = "", digits = 2, caption = "Summary of the linear mixed-effects model for testing the correlation between vowel and closure duration (see \\@ref(s:vcduration)).") %>%
kable_styling(font_size = 8, latex_options = "hold_position")
```
...
In \@ref(fig:FigureR-5), boxplots and raw data points show the duration of the release-to-release interval in words with a voiceless vs a voiced C2 stop, in Italian and Polish.
...
Recall, however, that the interaction between language and C2 voicing (which gives the estimate of 10.54) is not significant (see the full model summary in \@ref(tab:vow-table)).
...
\@ref(fig:FigureR-6} shows the random coefficients of the effect of C2 voicing on vowel duration for the individual speakers, extracted from the mixed-effects model presented in \Cref{s:vduration).
...
### Compensatory temporal adjustment {#s:comp}
...
\@ref(fig:compensatory) illustrates this compensatory mechanism.
...
<!-- \@ref(s:gestural) discusses the interpretation of the release-to-release invariance in CV́CV words as a consequence of the timing of C2 rather than of a holistic CV́C motor plan in which the release-to-release interval is held constant.
Although beyond the scope of this paper, disambiguating between these two interpretations on articulatory grounds is fundamental for a general understanding of a theory of gestural organisation (a promising venue of research might be the activation-spin model by @tilsen2013}). -->
