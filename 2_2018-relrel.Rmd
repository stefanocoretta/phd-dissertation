...
```{r Figure2, include=TRUE, fig.cap = "Raw data and boxplots of the duration in milliseconds of vowels in Italian (top row) and Polish (bottom row), for the vowels /a, o, u/ when followed by a voiceless or voiced stop", out.extra="width=\\linewidth", cache=TRUE}
durations_filtered %>%
...
\Cref{fig:Figure2} shows boxplots and raw data of vowel duration for the three vowels /a, o, u/ when followed by voiceless or voiced stops in Italian and Polish.
...
```{r Figure3, echo=FALSE, include=TRUE, fig.cap = "Raw data and boxplots of closure duration in milliseconds of voiceless and voiced stops in Italian (top row) and Polish (bottom row) when preceded by the vowels /a, o, u/", out.extra="width=\\linewidth", cache=TRUE}
durations_filtered %>%
...
\Cref{fig:Figure3} illustrates stop closure durations with boxplots and individual raw data points.
...
\Cref{fig:Figure4} shows for each vowel /a, o, u/ the individual data points and the regression lines with 95\% confidence intervals extracted from the mixed-effects model.
...
In \Cref{fig:Figure5}, boxplots and raw data points show the duration of the release to release interval in words with a voiceless vs. a voiced C2 stop, in Italian and Polish.
...
```{r Figure6, include=TRUE, fig.cap = "By-speaker random coefficients and error bars for the effect of C2 voicing on vowel duration, extracted from a mixed-effect model (Section 3.1)", out.extra="width=\\linewidth", cache=TRUE, warning=FALSE}
...
\Cref{fig:Figure6} shows the random coefficients of the effect of C2 voicing on vowel duration for the individual speakers, extracted from the mixed-effects model presented in \Cref{s:vduration}.
