...
Finally, I will introduce the notions of "descriptive category" and "comparative concept" \citep{haspelmath2010} to show that the lingusitic reading of voicing in this dissertation will be in the latter sense.

<!-- senses/readings of voicing, physical and linguistic sense -->

From a physical point of view, voicing can be defined either acoustically or phisio-anatomically.
Acoustically, voicing is the presence of periodicity in a speech signal.
The phisio-anatomical source of acoustic periodicity is the cyclic vibration of the vocal folds.
...
The initiation of vocal fold vibration requires that the air pressure of the cavity below the vocal folds (broadly speking, the lungs) is higher than that of the cavity above them (the oral tract).
...
Similarly, the absence of voicing with no cuncurrent adjustments to prevent initiation and maintanence of vocal fold vibration is known as *passive devoicing*.
...
Active adjustmenst require muscular activity.
...
*Active voicing* is vocal fold vibration with articulatory adjustements that ensure continuing vibration.

Sounds that are intended not to have vocal fold vibration (i.e. are inteded as voiceless) but that are characterised by favourable conditions for it tend to show passive voicing.
In these cases, articulatory asjustments can be put in place to counteract the presence of pasive voicing.
...
\citet{rothenberg1967} makes an important further distiction between purposive and non-purposive active articulatory adjustments.
...
If tongue root advancement is executed by muscular activity with an intent different from maintining voicing (for example, aiding the creation of a tongue contriction movement), then this would be classified as a *non-purposive* (active) gesture in regards to voicing.
...
All of these languages are, even if at an alledgedly different extent, voicing-effect languages.
In true-voicing languages, vowels are longer when followed by [voice] segments, while in aspirating languages vowels are longer when followed by underspecified segments (segments withouth [spread glottis]).
...
Comparative concepts are created by the linguist who performs cross-lingusitic analyses, and are not components of particular languages.
I assume here that traditional phonological categories like "voicing", "vowel height", "place of articulation", can be thought of either as (language-particular) descriptive categories or comparative concepts, depending on the scientific enterprise.\footnote{\citet{haspelmath2010} proposes to use capitalised names for descriptive categories (for example, "Italian Voicing"), but since this use is not common among phonologists/phoneticians, I will not adop it here.}
...
<!-- TODO: say that you won't specify the use (physical or lingusitic) unless ambigous -->
...
\Cref{s:resq} discusses in detail the research questions and a justificatio of methods.
...
Future work is warranted to ascertain the role of a gradient/continous operationalisation of voicing.
...
The Italian voicing contrast is usually described in terms of an oppoistion between voiceless unaspirated consonants and fully voiced consonants \citep{vagges1978, bortolini1995, pape2014, kirby2016}.
<!-- TODO: VOT -->
...
The application of generalised additive models on electroglottographic data allows us to obtain a dynamic and multidimentional view of vibratory properties of the vocal folds (\Cref{p:wavegram}).
This constitutes an improvement from methods that reduce the multidimentionality of fold vibration to a single measure, like the closed quotient.
...
This dissertation is also an example of how state-of-the-art research methods can be applied to lingusitic research.
...
In interpreting the results, more emphasis was given to estimation of parmeters in statistical models, and to the degree of uncertainty surrounding it.
...
The following chapter, \Cref{s:methods}, is overview of the reaserch methods of Study I (\Cref{s:studyi}) and Study II (\Cref{s:studyii}).
