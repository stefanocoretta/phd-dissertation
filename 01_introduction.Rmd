...
Finally, the chapter concludes with a discussion of the effects of aspiration and ejection on vowel duration, and how these can shed light on the voicing effect (\Cref{s:beyond}).
...
Nonetheless, the four papers are laid out according to an order partly based on the chronological sequence of the research but also considering the logical dependence of the hypotheses treated in them.
...
The term "voicing", as used in the literature on the voicing effect and related phenomena, can have substantially different referents.
...
The voicing-effect languages listed in \Cref{s:veintro} possess quite different phonation systems.
A major distinction can be drawn between so-called "true voicing" languages and "aspirating" languages \citep{beckman2013}.
True voicing languages make use of the distinctive (privative) feature [voice], and segments specified with [voiced] are characterised by active voicing (vocal fold vibration).
...
\citet{keating1984a} is an attempt to define "voicing" in such a way that even systems that are very dissimilar at the physical level can be groupped together.
...
This definitions abstracts away from the presence/absence of vocal fold vibration, and allows us to group together for example an aspirating language like English and a true-voicing one like Italian.
...
Across English varieties, the effect of voicing is greater with stops than with fricatives by a mean factor of 1.3.
...
The third aim was to carefully (re)consider previous and current results in light of the methodological debates related to the Open Science movement (\Cref{s:openscience}).
...
```{=latex}
\ctable[caption = Italian consonant phonemes(adapted from Kramer 2009).,
label = t:itacons,
]{llllll}{}{
FL
...
```{=latex}
\ctable[caption = Italian vocalic phonemes (adapted from Kramer 2009).,
label = t:itavow,
]{llll}{}{
\FL
...
<!-- TODO: upload UTI data and delete footnote -->
