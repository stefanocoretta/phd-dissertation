...
The second objective was to enlighten the debate on reported cross-linguistic differences by conducting an analysis which encompasses three related but contrasting languages.
...
The long-standing debate on the source of the voicing effect (what properties or biases in speech production and perception allow this effect to emerge in a language in the first place), evidenced by the different proposals discussed in [...], whether articulatory or perceptual, is evidence for the difficulty in pointing to a single cause of the differential duration of vowels followed by voiceless vs voiced stops.
...
Such an approach allows us to formulate an account that can be generalised and applied to other durational phenomena (related to phonation or not).
...
In light of the complexity of the durational effects reviewed in [...], I decided to limit the scope of the investigation to aspects of production, while keeping an open mind about perceptual factors, as discussed in [... discussion].
This choice was based in part on the paucity of recent articulatory data on the voicing effect in relation to for example acoustic and perceptual data, and in part on the greater number of production accounts of the voicing effect relative to perception accounts.
...
Furthermore, the production accounts reviewed in [...] deal either with oral (tongue) gestures or laryngeal gestures.
In order to potentially identify properties of these two types of gestures it followed as a natural choice to use ultrasound tongue imaging and electroglottography in combination with acoustics as three sources of data.
...
<!-- TODO: some questions span across the two stages, exploratory and confirmatory, like for example replication of previous results and comparison across languages -->

<!-- TODO: Why these languages? -->

<!-- need to say that what I am interested in is how the voicing effect emerges in the first place, and not how individual languages exploit or not the effect -->
...
Italian is traditionally ascribed to the syllable-timed class of rhythmic typology \citep{pike1945}, although properties stress-timed languages (like vowel reduction) can be observed in Italian, also depending on the regional variety \citep{white2009, giordano2010, pamies-bertran1999}.
...
Polish lexical stress is fixed on the penultimate syllable, with the only exceptions with ante-penultimate stress being loan words \citep{gussmann2007}.
