...
As discussed in \Cref{s:ve-phonologise}, the ratio between the duration of the closure and that of the vowel has been identified as one of the perceptual cues to voicing [@port1982; @lisker1986].
...
The duration of both the syllable and the word is affected by stop voicing [@chen1970; @jacewicz2009], and it is not clear why compensation within a word should necessarily target the pre-consonantal vowel and not other segments (these issues are discussed in more details in \Cref{p:relrel}).
...
The trend of vowels being longer when followed by aspirated stops challenges some of the accounts presented in \Cref{s:accounts}, as noted in @maddieson1976.
...
While @maddieson1976 argue against a compensatory effect between vowel and consonant duration, the data in @durvasula2012 are instead compatible with it (see \Cref{p:relrel}).
...
Another fundamental aspect of this research project is that it was developed in two stages: an exploratory (hypothesis-generating) stage, and a confirmatory (hypothesis-testing) stage (on the exploratory/confirmatory dichotomy, see @tukey1980 and \Cref{s:stand}).
...
The vocalic system in \Cref{t:itavow} is found in stressed syllables, although the status of the mid-high and mid-low contrast is not straightforward (especially for the back vowels), and the mid vowels show a high degree of geographical and idiosyncratic variation [@renwick2016].
...
```{r set-up, fig.cap="Schematics of the equipment set-up of Study I. Stabilisation headset not shown."}
...
The subject wore a metallic headset produced by @articulate2008 (\Cref{fig:headset}), which stabilises the position of the ultrasound probe (allowing free head movement), and the Velcro strap with the EGG electrodes around their neck.
...
```{r headset, fig.cap="Ultrasonic probe stabilisation headset."}
knitr::include_graphics("./img/headset.png")
```
...
Subsequent data processing followed the method described in @strycharczuk2015 (see also \Cref{p:tra} and \Cref{a:polar-gams}) and it was based on the upsampled (100 kHz) spline data.
...
```{r kine, fig.cap="Gestural landmarks of tongue movements in the word *putu*. The top panel shows tongue tip displacement, while the bottom panel shows tangential and absolute velocity."}
knitr::include_graphics("./img/velocity.png")
```
...
The models of Study I were conducted within a Neyman-Pearson (frequentist) framework of statistical inference [@perezgonzalez2015], while a Bayes factor analysis [@kass1995] was performed in those cases that demanded it (\Cref{p:relrel}).
