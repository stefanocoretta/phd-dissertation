...
\Cref{s:rationale} presents a more detailed discussion of the rational behind the research of this dissertation, and a summary and conceptual contribution of the main results.
...
This chapter is an overview of how these three questions have been addressed and of the results.
...
Finally, new hypotheses were generated by the exploratory phase, which justified a confirmatory study (Study II, \Cref{s:studyii}).
...
In order to potentially identify properties of these two types of gestures it seemed a natural choice to use ultrasound tongue imaging and electroglottography in combination with acoustics as three sources of data.
...
Given that the voicing effect (and related durational phenomena) has been prevalently if not exclusively defined and dealt with in terms of acoustic segmental durations, the same approach is used here, and analysis of acoustic durations will be at the core of the analyses presented in Part II.
...
A convenient way to investigate mechanic properties underlying the effect of voicing on vowel durations is to consider languages in which the effect has not been claimed to be phonologised (\Cref{s:ve-phonologise}).
...
The split between /ʊ/ and /ʌ/ (respectively \textsc{foot} and \textsc{strut} in Well's lexical set, \citealt{wells1982}) present in many varieties of English is not in Manchester English (as in Northern English more generally), so that there is a single vowel category realised as [ʊ] \citep{baranowski2015}.
...
The following paragraphs summarise the contribution of each original publication in Part II, while a full-fledged discussion of the holistic proposal developed here will be given in \Cref{s:gen-disc} of Part III.
...
In \Cref{a:ve-eng} of the Appendix, I relate the current results with those from previous work, by means of a meta-analytical study of the English voicing effect.
...
While experimental testing of vowel-to-vowel isochrony and vowel-consonant phasing is warranted, \Cref{a:gons} in the Appendix provides initial partial evidence.
...
In \Cref{p:wavegram}, the analysis of vocal fold activity during the production of vowels showed that the latter portion of vowels followed by voiceless stops is produced with greater glottal spread in Italian than in Polish.
...
\Cref{a:polar-gams} in the Appendix shows how generalised additive modelling can be used with tongue contour data in polar coordinates to control for a complex combination of effects.
...
All the research materials (data, code, documentation) are available for download \citep{coretta2020a}.
In interpreting the results, more emphasis was given to estimation of parameters in statistical models, and to the degree of uncertainty surrounding it.
