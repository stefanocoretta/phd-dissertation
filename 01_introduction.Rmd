...
Nonetheless, the four papers are laid out here according to an order partly based on the chronological sequence of the research but also considering the logical dependence of the hypotheses treated in them (see below [...]).
Finally, Part III summarises the results of this investigation by providing an overarching synthesis (\Cref{s:gen-disc}), and concludes with a discussion of limitations and future avenues of research (\Cref{s:final}).

<!-- TODO: expand on content of individual chapters and say that Paper I is the foundation coz experimental, then you have II which is confirming I in English, then III and IV expand on the issue of VC boundary placement -->
...
First, I will discuss how the voicing effect is cross-linguistically common (with the typological caveat that most investigated languages are from the Indo-European family), although alleged exceptions to its universality exists, both in terms of presence of the effect and of its magnitude.
This is followed by a discussion of the use of "voicing" as a comparative concept rather than as a phonetically-motivated descriptive category, and by a presentation of other phonological and phonetic factors that are known to interact with the voicing effect, such as manner, prosody, and the process of phonologisation.
...
For each language, a brief description of the consonantal and vocalic phonemic systems is given, with a focus on aspects of phonation contrasts, followed by a discussion of the stress and rhythmic systems.
\Cref{s:sumresults} offers a prospective overview of the main results, which presented in full in Part II.

The last chapter of Part I, \Cref{s:methods}, describes the methods employed in the exploratory study of the voicing effect in Italian and Polish (Study I) and in the confirmatory study of the compensatory aspects of the voicing effect in English (Study II).
...
Finally, \Cref{s:openscience} discusses issues related to statistical aspects of scientific research, and introduces principles and practices of Open Science as a remedy to some of these issues and how Open Science has shaped the current research project.
...
<!-- TODO: maybe remove this heading -->
...
<!-- TODO: move here research questions -->
...
## Italian, Polish, and English phonological systems
...
<!-- TODO: say that you are using names as languoids -->
...
Italian contrasts consonants along five (phonological) places of articulation: labial (bilabial and labiodental), dental, alveolar, palatal (palatal and post-alveolar), and velar.
...
<!-- TODO: VOT -->
\citet{pape2014} shows that Italian speakers tend to percieve stops without a burst following the release as voiced consonants, independent of the duration of voicing during closure.
While it is not clear which acustic cue is employed by Italian speakers to discriminate voiceless and voiced consonants, \citet{pape2014} find in their production study that Italian consistently articulate (velar) stops with full voicing during closure.

The vocalic system in \Cref{t:itavow} is found in stressed syllables, although the status of the mid-high and mid-low contrast is not straightforward (especially in the back vowels), and the mid vowels show a high degree of geographical and idiosyncratic variation \citep{renwick2016}.
...
\ctable[caption = British English vowel phonemes \citep{...}.,
label = t:polcons,
]{llll}{}{
\FL
     & front & central & back \ML
...
<!-- maybe include Norther British English vowels? -->

English consonant phonemes belong to one of seven places of articulation (labial, dental, alveolar, post-alveolar, palatal, velar, glottal) and seven manner of articulation (plosive, fricative, affricate, nasal, lateral, rhotic, approximant).
The obstruents (plosives, fricatives, affricates) contrast for what has been traditionally described as voicing (...), which is also reflected in the standard use of IPA voiceless and voiced symbols.
...
I have written a custom implementation of the procedur in Praat for this study (`release-detection-c1.praat` and `burst-detection.praat` in \citealt{coretta2018m}).
...
<!-- TODO: check that cross-references still make sense -->
See Paper III and IV for more details.
Tongue displacement was obtained by tracking displacement along fan-lines from a fan-like coordinate system \citep{scobbie2011}.
Displacement was measured for the tongue tip, tongue dorsum, and tongue root, broadly defined as the regions of the tongue that produce the closure for coronal stops and velar stops for respectively tongue tip and dorsum, and the region between the hyoid bone shadow and the tongue dorsum region for the tongue root.
Within each tongue region, the fin-line with the highest standard deviation was chosen as the vector for calculating tongue displacement (these fan-lines were manually chosen for each speaker individually).
...
Tangential and absolute velocity was calculated from this smoothed displacement.
