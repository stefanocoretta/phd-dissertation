...
The articulatory temporal stability across voicing contexts is evidenced in this dissertation by the durational stability of the interval between two consecutive stop releases.
...
Furthermoew, \citet{de-jong1991} finds only partial support for the independence of vowel and consonant gestures.
...
While the strong prediction of vowel-to-vowel isochrony is not confirmed by data comparing singleton and geminates, a softer formulation of cyclicity could work.
A possible reason for why the isochrony breaks in that context is that geminate consonant are a blend of two phasing patterns.
...
If this is the case, then isochrony would apply only in those cases where the intervening consonants are in-phase with the second vowel (see \Cref{s:final} for testable hypotheses).
...
While the timing of gestural onset is identical in both voicing contexts, the difference in closing velocity produces the acoustic pattern of the differential VC boundary timing.
...
Note, however, that no conclusive statement can be done in this regard, due to the low precision obtained for the relevant estimates.
...
Although the estimated range of values is between 55 and 95 ms, the analysis revealed a possibility for publication bias in favour of larger effects, meaning the the obtained values might be an overestimate.
...
\citet{tanner2019} argue that the effect they found in spontaneous speech is smaller than that of the laboratory speech reported in previous studies.
...
While the results of this endeavour contributed to answering the questions introduced in \Cref{s:overview}, some open issues remain and new questions are brought to light.
...
Moreover, nonce words eliminate issues in relation to usage factors like lexical frequency, which can have a substantial influence \citep{hay2015, soskuthy2017a, soskuthy2018, todd2019}.
...
Much of the phonetic literature relies on small sample sizes, but most work is done on speech data which is typically quite noisy.
