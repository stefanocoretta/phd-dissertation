...
\begin{figure}
  ...
  \caption{Gestural organisation patterns for onsets (a), codas (b), heterosyllabic onsets (c). C = consonant, V = vowel (design based on \citealt{marin2010}).}
...
In some cases, like in the cross-linguistic comparative analysis of the voicing effect, it is difficult to make unambiguous inferential statements.
Much of this uncertainty derives from the sample sizes employed in these studies.
Although the number of speakers included here is generally equal or above average (see \Cref{a:speakers}), the number of observations is not at times sufficient enough to reach an appreciable degree of precision.
...
For example, for a 90 ms vowel, speakers can reliably detect differences of 5 ms (cf. the Weber–Fechner law, \citet{fechner1966}).
...
The compensatory account presented here rests on the cyclic production of vowel sequences \citet{ohman1967, fowler1983}.
\citet{zmarich2011} and \citet{zeroual2015} have demonstrated that the interval between the vowels flanking the stops is not isochronous in the context of singleton vs. geminate stops.
However, the absence of isochrony in these contexts follows from the gestural organisation of the segments involved.
While in the case of singleton stops, the vocalic gestures are immediately adjacent to each other, in the case of geminates, there is an intervening (consonantal) gesture that is in an anti-phase relation with the first vowel in the sequence.
